...
| 2026-06-11 | **Fix: intake form submissions not reflected in-app.** Storage was fine (verified in Supabase — response rows + auto-created clients persist); the bug was the display pipeline. `submitIntakeResponse` now logs a `client_activity_log` row (`eventType: 'intake_submitted'`, entityType `note`, label = form title) so submissions appear in the client activity tab + global feed. `ActivityFeed` gained the `intake_submitted` case (violet `ClipboardText` icon + bg + "{client} submitted the intake form '{title}'"). New `getClientIntakeResponses(clientId, userId)` DB fn powers a **Submitted Forms** card on the client detail page (lists this client's responses, each linking to the response viewer). Added `/forms` → `/intake-forms` redirect in `next.config.mjs`. `ClipboardText` added to the icons re-export. | `src/lib/db/supabase.ts`, `src/components/clients/ActivityFeed.tsx`, `src/app/(dashboard)/clients/[id]/page.tsx`, `src/components/icons.tsx`, `next.config.mjs`, `CODEBASE.md` |
| 2026-06-11 | **Fix: missing email notification events + toggles.** Added 4 toggles to Settings → Notifications (`payment_received`, `proposal_accepted`, `proposal_declined`, `intake_submitted`). New shared `getNotificationRecipient(userId, key)` DB helper returns the freelancer email/name only when the toggle is on (default on), gating sends in one query. Wired triggers: proposal accept (existing email now gated + buildSenderName), proposal decline (new `proposal-declined` template + trigger — previously sent nothing), intake submit (new `intake-submitted` template, links to responses viewer), payment claim (gated on `payment_received`). All non-blocking. | `src/components/settings/SettingsTabs.tsx`, `src/lib/db/supabase.ts`, `src/app/api/intake/[token]/submit/route.ts`, `src/app/api/proposals/[id]/accept/route.ts`, `src/app/api/proposals/[id]/decline/route.ts`, `src/app/api/i/[token]/payment-claim/route.ts`, `src/lib/email/templates/intake-submitted.ts`, `src/lib/email/templates/proposal-declined.ts`, `CODEBASE.md` |
| 2026-06-13 | **Readable URLs + animated feature tours + standards-grade contract templates.** **URLs:** new `lib/slug.ts` — hybrid `slug--token` pattern (Notion/Figma style): `/portal/jane-cooper--{token}`, `/sign/website-design-agreement--{token}`, `/clients/jane-cooper--{uuid}`. Slug is display-only and ignored on lookup (token stays the access control, nothing enumerable); old plain-token links still work. Generators updated (portal copy/card/email, contract send returns `signPath`, clients list); consumers strip via `extractToken` (portal page, sign page + APIs, client pages + APIs). **Tours:** installed driver.js; new `FeatureTour` (brand-styled popovers, progress, per-browser seen memory, ✨ Tour button). Tours on Dashboard (auto-starts first visit), Proposals, Intake Forms, Time Tracking, Clients; `data-tour` anchors added. **Templates:** benchmarked vs AIGA Standard Form, Bonsai/Rocket Lawyer, The Legal Paige — every template gained Independent Contractor Status, Client Materials + Mutual Indemnification (variant-aware), Force Majeure (5 missing it), Governing Law → Dispute-Resolution ladder (negotiation→mediation→small-claims→courts + prevailing-party fees), and General Provisions (entire agreement, severability, ESIGN/UETA e-signature validity, survival). | `src/lib/slug.ts`, `src/app/portal/[token]/page.tsx`, `src/app/sign/[token]/page.tsx`, `src/app/api/sign/[token]/**`, `src/app/api/contracts/[id]/send/route.ts`, `src/app/api/clients/[id]/**`, `src/app/(dashboard)/clients/**`, `src/components/clients/**`, `src/components/contracts/{SendForSignatureButton,CopySigningLinkButton}.tsx`, `src/components/ui/FeatureTour.tsx`, `src/app/globals.css`, `src/app/(dashboard)/{dashboard,proposals,intake-forms}/page.tsx`, `src/components/time-tracking/TimeTrackingClient.tsx`, `src/components/intake-forms/SendIntakeFormButton.tsx`, `src/lib/contracts/templates.ts`, `package.json` (driver.js), `CODEBASE.md` |
| 2026-06-12 | **Big-feature batch.** **Structured payment methods:** new `lib/payment-methods.ts` (typed methods: bank/ACH/IBAN, PayPal, Stripe-style payment link, Venmo, Zelle, check, other + note; JSON envelope stored in existing `payment_instructions` column, legacy text still renders) + `PaymentMethodsEditor` in InvoiceForm; method cards render in InvoiceDocument "How to Pay", the public ClientPaymentPanel (clickable pay links), and plain-text in the PDF. **Milestone billing:** `MilestoneScheduleBuilder` on the invoice form — % or fixed milestones with presets (50/50, 30/30/40), allocation check, writes schedule into Payment Terms, optionally sets milestone 1 as the deposit. **Expenses→invoices:** `ExpenseImportButton` adds recorded expenses as labeled line items; `supabase/expenses_billable.sql` migration prepared for full billable tracking. **Rich text:** `RichTextEditor`/`RichTextContent` (markdown-lite toolbar: H1/H2/bold/italic/lists) for proposal summaries, rendered on public + dashboard proposal pages. **Portal integration:** portal contracts tab deep-links Review & Sign / View Signed via latest signing-session token (getClientPortalData now returns signToken). **Misc batch (prev commit):** portal button label, client detail 6 tabs, portal tabs, portal email copy, app-wide autofill off, real feedback API (old form was a fake stub), exports/imports/feedback redesigns, reminders marketing copy removed. | `src/lib/payment-methods.ts`, `src/components/invoices/{PaymentMethodsEditor,MilestoneScheduleBuilder,ExpenseImportButton,InvoiceForm,InvoiceDocument,ClientPaymentPanel}.tsx`, `src/lib/pdf/InvoicePDF.tsx`, `src/components/ui/RichText.tsx`, `src/components/proposals/ProposalForm.tsx`, `src/app/p/[id]/page.tsx`, `src/app/(dashboard)/proposals/[id]/page.tsx`, `src/app/portal/[token]/page.tsx`, `src/lib/db/supabase.ts`, `supabase/expenses_billable.sql`, `CODEBASE.md` |
| 2026-06-12 | **Bug-tracker sweep (18 fixes).** **Perf/blank pages:** route-group `loading.tsx` (skeleton) + `error.tsx` (retry boundary) for all dashboard pages — fixes blank screens after client creation, slow nav links, slow-feeling client creation. **Clients:** edit no longer wipes phone (getClientById now selects it); deleted dead stub ClientForm.tsx (fake-submit). **Proposals:** send route surfaces real email errors (502 + message) instead of bare 500; accept/decline log `proposal_accepted`/`proposal_declined` activity; dashboard shows accept/decline banners (new `getRecentProposalEvents`); /proposals shows "accepted — Create Contract" nudge banners; portal got tabs (Invoices/Contracts/Proposals) with proposals listed + Review CTA (extended `getClientPortalData`); "Offer Valid Until" relabeled + helper; button labels. **Contracts:** markdown corruption fixed (bold-first parsing in renderInlineParts x2, cleanLine, stripMarkdown — also protects ____ signature lines); signed contracts now render read-only with Download Signed PDF via new public `/api/sign/[token]/pdf`; first visit auto-applies profession template (new `templateForProfession()`); IP explanation in PDF audit + signed email; "Send Email" label; signature autocomplete off. **Onboarding:** 4 steps with new logo-upload step; email labeled as sign-in identity (+ Settings helper); `TRIAL_DAYS` corrected 30→5 (matches DB `now()+5 days`, landing footnote now uses constant); final step links remaining Settings; Skip dismisses instantly (background save). **Invoices:** number inputs blur-on-wheel globally (shared Input); time-log import empty state links to /time-tracking; /invoices got a Dashboard back link; tax-rate auto-populate + Duplicate button verified already working. | `src/app/(dashboard)/loading.tsx`, `src/app/(dashboard)/error.tsx`, `src/lib/db/supabase.ts`, `src/app/(dashboard)/**`, `src/app/portal/[token]/page.tsx`, `src/app/sign/[token]/page.tsx`, `src/app/api/sign/[token]/pdf/route.ts`, `src/app/api/proposals/[id]/{send,accept,decline}/route.ts`, `src/components/{clients,proposals,contracts,onboarding,settings,invoices,sign}/**`, `src/components/ui/input.tsx`, `src/lib/contracts/templates.ts`, `src/lib/pdf/ContractPDF.tsx`, `src/lib/utils.ts`, `src/lib/constants.ts`, `src/components/landing/LandingContent.tsx`, `CODEBASE.md` |
| 2026-06-12 | **Fix: all 5 email notification triggers audited + repaired.** contract_opened: no trigger existed → new `recordContractOpen()` (open_count, sent→opened, audit + activity, gated 1/hr email, new `contract-opened` template) wired into sign page. contract_signed: was unconditional → gated on toggle. invoice_opened: rate-limit was broken (matched reminder_logs.recipient_email against freelancer *name*) + dashboard URL used user_id → fixed via new `previousOpenedAt` on InvoicePublicRow + correct URL + getNotificationRecipient. invoice_overdue: no trigger → `markOverdueInvoices` now returns `OverdueTransition[]` + logs activity; emails sent once per transition from cron (new `invoice-overdue` template). daily_summary: no cron existed → new `/api/cron/daily-summary` @ 8:30am UTC (vercel.json) sends digest (overdue list, outstanding total, reminders due today) only when non-empty (new `daily-summary` template). | `src/lib/db/supabase.ts`, `src/app/sign/[token]/page.tsx`, `src/app/api/sign/[token]/route.ts`, `src/app/i/[token]/page.tsx`, `src/app/api/cron/daily-summary/route.ts`, `src/lib/email/templates/contract-opened.ts`, `src/lib/email/templates/invoice-overdue.ts`, `src/lib/email/templates/daily-summary.ts`, `vercel.json`, `CODEBASE.md` |
| 2026-06-11 | **Feature: send intake form to client + preview as client.** Added a **Send** button per form on the intake-forms list → dialog to pick a saved client (email pulled server-side) or enter an email manually → POSTs `/api/intake-forms/[id]/send` which emails the public form link (new `intake-form` template, reply-to = freelancer). Added a **Preview as client** button in the builder's Preview card → modal rendering the form exactly as `/intake/[token]` shows it (new `ClientFormPreview`, read-only, mirrors `IntakeSubmitForm`). | `src/app/(dashboard)/intake-forms/page.tsx`, `src/components/intake-forms/SendIntakeFormButton.tsx`, `src/components/intake-forms/ClientFormPreview.tsx`, `src/components/intake-forms/IntakeFormBuilder.tsx`, `src/app/api/intake-forms/[id]/send/route.ts`, `src/lib/email/templates/intake-form.ts`, `CODEBASE.md` |
