...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  guides: 'bg-[#0A2540]/50 text-[#A5B4FC] border-[#635BFF]',
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {posts.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="group rounded-xl border border-[#1A3A5C] bg-[#071929] p-6 hover:border-[#635BFF] transition-colors"
              >
                ...
                <h2 className="font-semibold text-white text-sm leading-snug mb-2 group-hover:text-[#A5B4FC] transition-colors">
                  {post.title}
                </h2>
                <p className="text-gray-500 text-xs leading-relaxed line-clamp-3">{post.description}</p>
                <p className="text-[#635BFF] text-xs mt-4 font-medium">Read article →</p>
              </Link>
            ))}
          </div>
        )}
