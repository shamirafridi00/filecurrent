...
function getEventIcon(eventType: string) {
  switch (eventType) {
    ...
    case 'proposal_accepted':    return <CheckCircle size={16} className="text-emerald-500" />
    case 'proposal_declined':    return <Warning size={16} className="text-red-400" />
...
function getEventIconBg(eventType: string): string {
  ...
  if (eventType === 'proposal_accepted') return 'bg-emerald-50 border-emerald-100'
  if (eventType === 'proposal_declined') return 'bg-red-50 border-red-100'
...
function getEventDescription(event: ClientActivityLogRow): string {
  ...
  switch (event.event_type) {
    ...
    case 'proposal_accepted':    return `${client} accepted your proposal "${label}"${amt ? ` — ${amt}` : ''}`
    case 'proposal_declined':    return `${client} declined your proposal "${label}"`
